...
json_file = open('2DCNN_model','r')#1DCNN_model or 2DCNN_model
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("2Dmodel.h5") #1Dmodel.h5 or 2Dmodel.h5
data = pd.read_csv("C:/Users/krarv/Desktop/ProjectData/SampleData/Sample{}Data.csv".format(sample_number),header=None,delimiter=',')
x = data
#x = x.values.reshape(x.shape[0],x.shape[1],1) #Use this reshape when predicting using 1D CNN
x = x.values.reshape(x.shape[0],4,4,1) #Use this reshape when predicting using 2D CNN
    ...
    for col in range (800):
        for rgb in range (3):
            if(mask[row][col][rgb]> 0):
                predictedImage[row][col] = (255,255,255)
...
plt.title('Sample {} - Intensity Image(wavelength={},Dimension=1)'.format(sample_number,wavelength)), plt.xticks([]), plt.yticks([])
plt.show()